...
Three-layer infrastructure stack: Ansible bootstraps the KVM hypervisor, OpenTofu provisions VMs, Flux CD manages the Kubernetes cluster.
...
- Two-phase: `prepare-host.yaml` (as root) then `site.yaml` (as ansible user) — separate because the ansible user does not exist during phase 1
...
- Two distinct users: `libvirt_user` (management account, gets polkit rule) vs `libvirt_qemu_user` (QEMU process user set in qemu.conf) — do not conflate
- Integer qemu.conf settings (`dynamic_ownership`) must be unquoted integers — separate task from the string settings loop
...
- Module hierarchy: `libvirt-vm` ← `ubuntu-vm` / `talos-vm`; `talos-cluster` is independent (no libvirt)
- Every module declares `required_providers` in its own `versions.tofu` — OpenTofu does not inherit provider source from the root
...
- Apply order for Talos: `kvm-talos-xx apply` first, `talos apply` second (reads kvm-talos-xx statefile via `terraform_remote_state`)
...
- Cilium settings must be kept in sync between the Talos inline manifest (`cilium-values` ConfigMap) and `k8s/infrastructure/cilium/helmrelease.yaml` — they are applied at different lifecycle phases
